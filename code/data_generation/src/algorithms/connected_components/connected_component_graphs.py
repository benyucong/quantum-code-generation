...
def generate_graphs(max_nodes, min_components, max_components, iterations):
    graphs = []
    graph_hashes = []
    ...
    for n_components in range(min_components, max_components + 1):
        ...
        for _ in range(iterations):
            G, components = generate_graph_with_components(n_components, min_size=2, max_size=max_component_size, edge_prob=0.5)
...
def visualize_graphs(graphs, num_graphs=10, output_dir="algorithms/connected_components/figures"):
    for i in range(num_graphs):
        G, node, components = random.choice(graphs)
        plt.figure()
        nx.draw(G, with_labels=True, font_weight='bold')
        plt.title(f"Graph with {len(G)} nodes and {len(components)} components\nEdge: {node}")
