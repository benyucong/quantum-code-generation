...
def visualize_graphs(graphs):
    for i in range(10):
        ...
        nx.draw(G, pos, node_size=80, edge_color="gray", with_labels=False, node_color=[coloring[node] for node in G.nodes()])
            ...
            if n_colors * n_nodes < 21:
                for _ in range(2000):
                    for prob in range(2, 10):
                        inter_prob = prob / 10
                        G, coloring = generate_k_colorable_graph(n=n_nodes, k=n_colors, inter_prob=inter_prob)
                        hash_val = weisfeiler_lehman_graph_hash(G)
                        if len(G.nodes()) == n_nodes and nx.is_connected(G) and hash_val not in graph_hashes:
                            print(f"Generated a {n_nodes}-node, {n_colors}-colorable graph")
